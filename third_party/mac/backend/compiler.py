...
from triton._C.libtriton import ir, mac
...
class MACOptions:
    ...
    def hash(self):
        ...
        libs_key = "0.1"
        return hashlib.sha256(libs_key.encode("utf-8")).hexdigest()
...
class MACBackend(BaseBackend):
    ...
    def parse_options(self, opts) -> Any:
        args = {k: opts[k] for k in MACOptions.__dataclass_fields__.keys() if k in opts}
        # args["allow_fp8e4nv"] = self.capability >= 89
        # args["allow_fp8e4b15"] = self.capability < 90
        # args["max_num_imprecise_acc_default"] = 2**30 if self.capability == 90 else 0
        return MACOptions(**args)
    ...
    #     mac.passes.ttnvgpuir.add_plan_cta(pm, cluster_info)
    ...
    #     passes.common.add_symbol_dce(pm)
    #     if capability // 10 >= 9:
    #         mac.passes.ttnvgpuir.add_fence_insertion(pm)
    #         mac.passes.ttnvgpuir.add_tma_lowering(pm)
    #     passes.common.add_canonicalizer(pm)
    #     pm.run(mod)
    ...
    #     pm = ir.pass_manager(mod.context)
    #     pm.enable_debug()
    #     mac.passes.ttgpuir.add_decompose_unsupported_conversions(pm)
    #     passes.ttgpuir.add_combine_tensor_select_and_if(pm)
    #     passes.convert.add_scf_to_cf(pm)
    #     passes.convert.add_index_to_llvmir(pm)
    #     passes.ttgpuir.add_allocate_shared_memory(pm)
    #     mac.passes.ttgpuir.add_to_llvmir(pm, capability)
    #     mac.passes.ttnvgpuir.add_nvgpu_to_llvm(pm)
    ...
    #     mac.set_nvvm_reflect_ftz(llvm_mod)
    ...
    @functools.lru_cache()
    def hash(self):
        version = "0.0.1" # get_ptxas_version()
